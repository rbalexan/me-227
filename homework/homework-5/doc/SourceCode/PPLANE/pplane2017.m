function output = pplane2017(action,input1,input2,input3)

% pplane2017  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane2017 is executed, a pplane2017 Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane2017
...
%	the pplane2017 Display window, the solution to the system with that
...
if strcmp(action,'initialize')
   ...
   % First we make sure that there is no other copy of pplane2017
   % running, since this causes problems.	
   pph = findobj('tag','pplane2017');
   if ~isempty(pph);
      qstring = {'There are some pplane2017 figures open although they may be invisible.  ';...
            'What do you want to do?'};
      tstring = 'Only one copy of pplane2017 can be open at one time.';
      b1str = 'Restart pplane2017.';
   ...
   % Next we look for old files created by pplane2017.
   ...
   for k = 1:length(oldfiles)
      ...
      ll = fgetl(fid);
      fclose(fid);
      if strcmp(ll,'%% Created by pplane2017')
         delete(fn)
      else
	l= length(ofiles);
	ofiles{l+1} = fn;
  ...
  ud.color = color;
  ud.level = ' ';
  ppset = figure('name','pplane2017 Setup',...
		 'tag','pplane2017','visible','off',...
		 'user',ud);
  
  pplane2017('figdefault',ppset);
  ...
  butt(2) = uicontrol('style','push',...
		      'pos',rwind,...
		      'string','Revert',...
		      'CallBack','pplane2017(''revert'')',...
		      'visible','off');
  
  butt(3) = uicontrol('style','push',...
		      'pos',pwind,...
		      'string','Proceed',...
		      'CallBack','pplane2017(''proceed'')',...
		      'visible','off');
  ...
  callnfpts = [
      'ppset = findobj(''name'',''pplane2017 Setup'');',...
      'ud = get(ppset,''user'');'...
      'me = ud.h.npts;',...
  ...
  meload = uimenu(mefile,'label','Load a system ...',...
		  'CallBack','pplane2017(''loadsyst'',''system'');',...
		  'pos',1);
  mesave = uimenu(mefile,'label','Save the current system ...',...
		  'CallBack','pplane2017(''savesyst'',''system'');',...
		  'pos',2);
  meloadg = uimenu(mefile,'label','Load a gallery ...',...
		   'CallBack','pplane2017(''loadsyst'',''gallery'');',...
		   'separator','on','pos',3);
  mesaveg = uimenu(mefile,'label','Save a gallery ...',...
		   'CallBack','pplane2017(''savesyst'',''gallery'');',...
		   'tag','savegal','pos',4);
  ...
  melddg = uimenu(mefile,'label','Load the default gallery',...
		  'CallBack','pplane2017(''loadsyst'',''default'');',...
		  'enable','on',...
  ...
  merestart = uimenu(mefile,'label',...
		     'Restart pplane2017',...
		     'CallBack','pplane2017(''restart'')',...
		     'separator','on','pos',12);
  
  mequit = uimenu(mefile,...
		  'label','Quit pplane2017',...
		  'CallBack','pplane2017(''quit'')',...
  ...
  % Gallery menu.
   ...
   meadd = uimenu(sysmenu,'label','Add current system to the gallery',...
      'CallBack','pplane2017(''addgall'');','tag','add system');
   sep = 'on';
   for kk = 1:length(system)
      kkk = num2str(kk);
      if kk == 2, sep = 'off';end
      sysmen(kk) = uimenu(sysmenu,'label',system(kk).name,...
         'CallBack',['pplane2017(''system'',',kkk,')'],...
...
elseif strcmp(action,'loadsyst')  % This loads either a system or a gallery.
   ...
   for j = 1:nnn
      ...
      newsyst.name = sname;
      ignore = pplane2017('addgall',newsyst);
      if ignore == -1;
...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'user',sud);
   he = findobj('name','pplane2017 Equilibrium point data');
   hl = findobj('name','pplane2017 Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
   ...
   if (sud.flag == 1)
     ...
     NumbFPts = sud.c.npts;
     ppdisp = findobj('name','pplane2017 Display');
     dud = get(ppdisp,'user');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
	 ...
	 if isempty(pv) 
	   ...
	   if isempty(str2num(pv)) % This is an expression.
	     ...
	     if (err | isempty(pval))
	       ...
	       fprintf('\a')
	       errordlg(errmsg,'pplane2017 error','on');
	       warning(SS)
	       return
	     end
	   end
	   xxderivstr = pplane2017('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane2017('paraeval',pn,pv,yderivstr);
     ...
     % Find pplane2017 Display if it exists.	
     % If pplane2017 Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','pplane2017 Display',...
		       ...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','pplane2017');
       pplane2017('figdefault',ppdisp);
       dud = get(ppdisp,'user');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       dbutt(2) = uicontrol('style','push',...
			    ...
			    'string','Quit',...
			    'CallBack','pplane2017(''quit'')','visible','off');
       
       dbutt(3) = uicontrol('style','push',...
			    'pos',[buttl,ppaxb+buttsep,buttw,butth],...
			    'string','Print',...
			    'CallBack','pplane2017(''print'')','visible','off');	
       ...
       merestart = uimenu(fmenu,'label',...
			  ...
			  'CallBack','pplane2017(''restart'')',...
			  'separator','on');
       mequit = uimenu(fmenu,'label','Quit pplane2017',...
		       'CallBack','pplane2017(''quit'')','separator','off');
       ...
       menu(2) = uimenu(emenu,'label','Zoom in.',...
			'CallBack','pplane2017(''zoomin'')',...
			'pos',1);
       
       zsqmenu = uimenu(emenu,'label','Zoom in square.',...
			'CallBack','pplane2017(''zoominsq'')',...
			'pos',2);
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       menu(6) = uimenu(menu(1),'label','Make the Display Window inactive.',...
			'CallBack','pplane2017(''hotcold'')','separator','on');
	   ...
	   'CallBack','pplane2017(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','pplane2017(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','pplane2017(''cdisp'')');
	...
	case 6
     ...
     ppkbd = findobj('name','pplane2017 Keyboard input','vis','on');
     if ~isempty(ppkbd),pplane2017('kbd'),end
     pplevel = findobj('name','pplane2017 Level sets');
...
elseif strcmp(action,'linear')
   ...
   % Get the information from pplane2017 Equilibrium ...
   ...
   % Find pplane2017 Linearization if it exists.	
   % If pplane2017 Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','pplane2017 Linearization');
   if (~isempty(pplin))
      figure(pplin);
      dud = get(pplin,'user');
      dud.syst = system;
      ...
      pplin = figure('name','pplane2017 Linearization',...
         ...
         'interrupt','on',...
         'visible','off',...
         'tag','pplane2017');
      pplane2017('figdefault',pplin);
      dud = get(pplin,'user');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      mequit = uimenu(fmenu,'label','Quit pplane2017',...
	  'CallBack','pplane2017(''quit'')','separator','on','pos',9);
	...
	chkd = 'on';
      ...
      set(pplin,'WindowButtonDownFcn',['pplane2017(''down'',',num2str(pplin),')']);
...
elseif strcmp(action,'dirfield')
   ...
   % Find pplane2017 Display and get the user data.
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'pplane2017 Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','pplane2017(''down'')');
...
elseif strcmp(action,'down')
  ...
  pplane2017('solution',initpt,disph);
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      pplane2017('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      pplane2017('solution',initpt,ppdisp);
   end  
   pplane2017('solution',[1,-1],ppdisp);
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % button on the pplane2017 Keyboard figure.
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane2017('paraeval',pn,pv,xstr);
	 ystr = pplane2017('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'kbd')
   ...
   % sets up the pplane2017 Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','pplane2017 Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','pplane2017 Keyboard input',...
		  'vis','off',...
		  'tag','pplane2017');
   
   pplane2017('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','CallBack','pplane2017(''kcompute'')');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','pplane2017 Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','pplane2017 Equilibrium point data',...
 		    'tag','pplane2017');
     pplane2017('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   set(gcf,'WindowButtonDownFcn','pplane2017(''zoom'')',...
...
elseif strcmp(action,'zoom')
   ...
   set(axh,'user',aud);	
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane2017(''down'')',...
      'WindowButtonUpFcn','');
   pplane2017('dirfield',disph);
   ppset = findobj('name','pplane2017 Setup');
   if isempty(ppset)
      pplane2017('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
	...
	'clickedcallback','pplane2017(''print'');');
    switch name
     case 'pplane2017 Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedcallback',['pplane2017(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedcallback',['pplane2017(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'pplane2017 Linearization'
...
elseif strcmp(action,'settings')
   ...
   % up the pplane2017 Settings window, which allows the user to interactively
   ...
   setfig = findobj('name','pplane2017 Settings');
   if ~isempty(setfig);
     delete(setfig);
   end
   
   setfig = figure('name','pplane2017 Settings',...
		   ...
		   'tag','pplane2017','vis','off');
   
   pplane2017('figdefault',setfig);
...
elseif strcmp(action,'setchange')
   ...
   % on the pplane2017 Settings window.
...
elseif strcmp(action,'plotxy')  % Start a graph 
  ...
  x=get(objh,'xdata');
  y=get(objh,'ydata');
  hh = pplane2017('plotxyfig',type,gcf);
  set(ppdisp,'WindowButtonDownFcn',oldcall);
  aud = get(hh(2),'user');
  ud = get(hh(1),'user');
  ud.data = [t;x;y];
  set(hh(1),'user',ud);
  pplane2017('plxy',aud.rad,hh(1));
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('NumberTile','off',...
		'tag','pplane2017','visible','off');
  name = ['pplane2017 t-plot # ',int2str(ppxy)];
  ud.color = dud.color;
  ud.type = 0;
  set(ppxy,'name',name,'user',ud);
  pplane2017('figdefault',ppxy);
  ...
  set(findobj(fmenu,'label','Print Pre&view...'),'pos',6);
  set(findobj(fmenu,'label','&Print...'),'pos',7);
  qmenu = uimenu(fmenu,'label','Quit pplane2017',...
		 'CallBack','pplane2017(''quit'')',...
		 'separator','on');
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','pplane2017(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = pplane2017('plotxyfig',type,ppxy);
   ud = get(hh(1),'user');
   ud.data = [t;x;y];
   set(hh(1),'user',ud);
   aud = get(hh(2),'user');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane2017(''plxybmf'')',...
      'windowbuttonupfcn','pplane2017(''plxybuf'')',...
      'inter','on');
   aud = get(dispa,'user');
   delete(aud.h);
   aud.h = [];
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the pplane2017 Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the pplane2017 Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'figdefault')
  ...
  fig = input1;
  set(fig,'CloseRequestFcn','pplane2017(''closefcn'')');
  ppset = findobj('name','pplane2017 Setup');
  sud = get(ppset,'user');
  ud = get(fig,'user');
  ud.ssize = sud.ssize;
...
elseif strcmp(action,'zoominsq')
   ...
   set(gcf,'WindowButtonDownFcn','pplane2017(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','inter','on');
   set(gca,'inter','on');
   dud = get(gcf,'user');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'user',aud);
   set(gcf,'windowbuttonmotionfcn','pplane2017(''zoomsqm'')',...
      'windowbuttonupfcn','pplane2017(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane2017(''down'')',...
      'WindowButtonMotionFcn','pplane2017(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane2017('dirfield',disph);
   ppset = findobj('name','pplane2017 Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','pplane2017 Level sets',...
		 'vis','off',...
		 'tag','pplane2017');
  
  pplane2017('figdefault',pplevel);
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane2017('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
elseif strcmp(action,'closefcn')
   ...
   fig = gcf;  
   name = get(fig,'name');
   if strcmp(name,'pplane2017 Setup') | strcmp(name,'pplane2017 Display')
      quest = ['Closing this window will cause all pplane2017 ',...
            'windows to close, and pplane2017 will stop.  ',...
            'Do you want to quit pplane2017?'];
      butt = questdlg(quest,'Quit pplane2017?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         pplane2017('quit');
      end
   elseif strcmp(name,'pplane2017 Linearization')
      dud = get(fig,'user');
      fcn = dud.function;
...
elseif strcmp(action,'confused')
   ...
   tstring = 'pplane2017 is totally confused';
   qstring = {['You will have to restart pplane2017 from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart pplane2017.';
   bstr2 = 'Just quit pplane2017.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','pplane2017'));
      pplane2017;return
...
% Input the user data. 
  ...
  tout = tout(1:N);
  yout = yout(1:N,:);
  if ishghandle(dud.notice) & ishghandle(dud.noticeflag)
    nstr = get(dud.notice,'string');
...
% PPOUT  is the output function for pplane2017.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','pplane2017 Display'); %gca;   % 
dud = get(ppdisp,'user');
ppdispa = dud.axes;
dfcn = dud.function;
ud = get(ppdispa,'user');
